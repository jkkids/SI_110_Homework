...
CACHE_FNAME = 'cached_tweets.json' # String for your file. We want the JSON file type, bcause that way, we can easily get the information into a Python dictionary!

try:
    file = open(CACHE_FNAME, 'r') # Try to read the data from the file
    contents = file.read()  # If it's there, get it into a string
    CACHE_DICTION = json.loads(contents) # And then load it into a dictionary
...
##		to search for is. 
def getTweets(item):
	if item in CACHE_DICTION:
		print("Found in cached tweets")
		return CACHE_DICTION[item]
	else:
		print ("Searching for item on twitter")
		data = api.search(q=item)
		try:
			CACHE_DICTION[item] = data
			...
			print("Can't find data in neither API nor cache")
			return None
...
for terms in range(3):
	term = input("Enter the term you would like to search for:")
	results = getTweets(term)
